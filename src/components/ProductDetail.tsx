...
export const ProductDetail: React.FC<ProductDetailProps> = ({
  ...
  const sellerProductsCount = (allProducts || []).filter(p => p.sellerId === seller.id).length;

  return (
    <div className="space-y-6 animate-fade-in text-zinc-100 mt-2 max-w-2xl mx-auto">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-5">
          <div className="relative w-full aspect-[16/10] sm:aspect-[16/9] overflow-hidden flex items-center justify-center bg-black/40 keep-bg-dark rounded-2xl border border-zinc-850">
            ...
            {allImages.length > 0 ? (
              isVideoUrl(allImages[currentSlideIndex]) ? (
                <video
        ...
        {/* 2. DETAIL HARGA (Title, Price, category, stock) */}
        <div className="bg-[#111827] rounded-3xl p-6 border border-zinc-800/50 shadow-xl space-y-5">
          ...
          <div className="text-3xl font-black text-[#0084ff] leading-none">
            {new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(currentDisplayPrice)}
          </div>
          
          <div className="text-xs text-zinc-400 font-medium">
            Stok: <span className="text-zinc-300">{currentDisplayStock} tersedia</span> · {formatSoldQty(soldQty)} Terjual
          </div>
          ...
          >
            ...
            <div className="px-5 pb-3 border-b border-zinc-850 flex items-center justify-between shrink-0">
              <div>
                ...
                <p className="text-xs font-black text-zinc-200 mt-0.5 truncate max-w-sm">{currentDisplayTitle}</p>
              </div>
              <button 
                onClick={() => setShowFullDesc(false)}
